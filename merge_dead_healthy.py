...
def transform_bounding_boxes(images, imnames, annotations, repeat_range, max_crop_size=(100, 100), im_type='healthy'):
    ...
    for stall_idx, stall in enumerate(empty_stalls):
        ...
        for i in random_choose:
            ...
            if crop.shape[0] <= stall.shape[0] and crop.shape[1] <= stall.shape[1]:
                ...
                segment = np.array(segmentation1, int)[0]
                segment = np.reshape(segment, (round(segment.shape[0] / 2), 2))
                segment = np.reshape(segment, (-1, 1, 2))

                #img1 = cv2.polylines(stall, segment, True, (255, 0, 0), 2)
                if label == 3:
                    img1 = cv2.polylines(stall, segment, True, (0, 0, 255), 2)
                    cv2.imshow('test', img1)
                    cv2.waitKey(0)

                annotations.append({
                    "id": ann_id,
                    "image_id": int(stall_idx+1),
                    "category_id": label,
                    "segmentation": segmentation1,
                    "area": area,
                    "bbox": [min(xn), min(yn), max(xn)-min(xn), max(yn)-min(yn)],
                    "iscrowd": 0
                })
                ann_id += 1
...
def combine_images_with_stalls(empty_stalls, healthy_anot, dead_anot, healthy_repeat_range, dead_repeat_range,
    ...
    healthy_crops, healthy_segmentations, export_areas = transform_bounding_boxes(healthy_images, himnames, healthy_anot, healthy_repeat_range, max_crop_size, im_type='healthy')

    # Transform dead chickens
    dead_crops, dead_segmentations, export_areas = transform_bounding_boxes(dead_images, dimnames, dead_anot, dead_repeat_range, max_crop_size, im_type='dead')

    # Initialize the annotation ID
    ann_id = 1

    # Place healthy chickens on empty stalls
    final_images, healthy_annotations, ann_id = place_bounding_boxes_on_stalls(empty_stalls, healthy_crops,healthy_segmentations, export_areas, healthy_repeat_range,label=1, starting_id=ann_id)

    # Place dead chickens on empty stalls
    final_images, dead_annotations, ann_id = place_bounding_boxes_on_stalls(empty_stalls, dead_crops, dead_segmentations, export_areas,
...
if __name__ == "__main__":
    ...
    healthy_image_dir =r'C:\Users\User\PycharmProjects\chickenLocalization\ChickenLocalization-main\healthy_images'
    dead_image_dir = r'C:\Users\User\PycharmProjects\chickenLocalization\ChickenLocalization-main\dead_images'
    empty_stall_dir = r'C:\Users\User\PycharmProjects\chickenLocalization\ChickenLocalization-main\empty_stalls'
    healthy_annotations_file = r'C:\Users\User\PycharmProjects\chickenLocalization\ChickenLocalization-main\healthy_annotations\annotations\annotations.json'
    dead_annotations_file = r'C:\Users\user\PycharmProjects\chickenLocalization\ChickenLocalization-main\dead_annotations\annotations\annotations.json'
    ...
    final_images, annotations = combine_images_with_stalls(empty_stalls, healthy_annotations,
                                                           ...
                                                           dead_repeat_range, max_crop_size=(100, 100))  # Set equal size

    # Create the output directory
    output_dir = 'resultimage2'
    os.makedirs(output_dir, exist_ok=True)

    # Save the final images and annotations
    image_metadata = []
    for idx, img in enumerate(final_images):
        image_filename = os.path.join(output_dir, f'final_image_{int(idx + 1)}.jpg')
        cv2.imwrite(image_filename, img)
        image_metadata.append({
            "id": int(idx + 1),
            "file_name": f'final_image_{int(idx + 1)}.jpg',
            "width": img.shape[1],
            "height": img.shape[0]
        })
    ...
    categories = [
        {"id": 0, "name": "healthy_chicken"},
        {"id": 1, "name": "dead_chicken"}
    ]
