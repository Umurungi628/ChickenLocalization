...
def load_images(image_dir):
    ...
    imnames = []
    # Loop through all .jpg files in the directory and add their file names to the list
    [imnames.append(str(os.path.split(file)[1])) for file in glob.glob(os.path.join(image_dir, '*.jpg'))]
    # Load and return the images and their names
...
def enhance_crop(crop, alpha=1.5, beta=40):
    # Enhance the crop by adjusting its contrast (alpha) and brightness (beta)
    enhanced = cv2.convertScaleAbs(crop, alpha=alpha, beta=beta)

    # Apply a sharpening kernel to make the crop edges more distinct
...
# Function to get bounding boxes and segmentations from annotations
def getimbboxes(imname, annotations):
    ...
    bboxes = []
    segmentations = []
    # Get image ID based on the image name
    [im_id.append(j['id']) for j in im_part if j['file_name'] == imname]
    # Get bounding boxes and segmentations for this image
...
# Function to get the coordinates of a polygon
def get_poly_coords(polygon):
    pol_area = []
    # Check the type of the geometry (Polygon, MultiPolygon, or GeometryCollection)
...
# Function to calculate intersection of coordinates
def intersect_coords(x1, y1, w, h):
    # Create a polygon representing the image area
    pim = Polygon(np.array([[0, 0], [w, 0], [w, h], [0, h]]))
    ppo = Polygon(np.array([x1, y1]).transpose()).buffer(0)
    ppo.buffer(0.01)
    if pim.intersects(ppo) is True:
        try:
            # If there is an intersection, compute the intersection polygon coordinates
...
def transform_bounding_boxes(images, imnames, annotations, repeat_range, max_crop_size=(80, 80), im_type='healthy'):
    ...
    # Iterate through each image and process its bounding boxes and segmentations
        ...
        if im_type == 'healthy':
            ...
            for _ in range(random.randint(*repeat_range)):
                ...
                if isinstance(bbox, list) and len(bbox) == 4:
                    ...
                    # Crop dimensions limiting
                    if x > 0:
                        crop = image[y:y + h, x:x + w]
                        crop = cv2.resize(crop, max_crop_size)  # Resize crop to max crop size
                        xc = xc * (max_crop_size[0] / w)  # Resize x-coordinates
                        yc = yc * (max_crop_size[1] / h)  # Resize y-coordinates
                        w, h = max_crop_size[0], max_crop_size[1]

                        # Apply enhancements like sharpening and contrast adjustment
...
if __name__ == "__main__":
    ...
    healthy_image_dir = r'C:\Users\User\PycharmProjects\chickenLocalization\healthy_images'
    dead_image_dir = r'C:\Users\User\PycharmProjects\chickenLocalization\dead_images'
    empty_stall_dir = r'C:\Users\User\PycharmProjects\chickenLocalization\empty_stalls'
    ...
    # Set the repeat range for bounding box placements
    healthy_repeat_range = (1, 5)
    dead_repeat_range = (1, 3)
